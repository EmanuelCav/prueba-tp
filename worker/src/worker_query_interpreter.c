...
void query_interpretar(char *line, int query_id, char *path_query, t_log *logger, t_memoria_interna *memoria, t_worker_config *cfg, int sock_master)
{
    ...
    case INS_CREATE:
        ...
        sprintf(comando_create, "CREATE|%s|%s|0", file, tag);
    ...
    case INS_TRUNCATE:
        ...
        sprintf(comando_truncate, "TRUNCATE|%s|%s|%d", file, tag, tamanio);
    ...
    {        
        ...
        sprintf(comando_tag, "TAG|%s|%s|%s|%s", file_origen, tag_origen, file_dest, tag_dest);
    ...
    case INS_COMMIT:
        ...
        sprintf(comando_commit, "COMMIT|%s|%s", file, tag);
    ...
    case INS_DELETE:
        ...
        sprintf(comando_delete, "DELETE|%s|%s", file, tag);
    ...
    case INS_END:
        enviar_comando_master(sock_master, logger, query_id, "QUERY_END", "");
...
void escribir_memoria(t_memoria_interna *memoria, int direccion, const char *contenido, t_log *logger, int query_id)
{
    ...
    for (int i = 0; i < memoria->cant_marcos; i++) {
        if (memoria->marcos[i].ocupada && memoria->marcos[i].numero_pagina == numero_pagina) {
            ...
            log_info(logger, "## Query %d: Escrito en memoria - Marco %d, Offset %d: %s", 
                     query_id, i, offset, contenido);
            return;
        }
    }
...
void leer_memoria(t_memoria_interna *memoria, int direccion, int tamanio, t_log *logger, int query_id, int sock_master)
{
    ...
    for (int i = 0; i < memoria->cant_marcos; i++) {
        if (memoria->marcos[i].ocupada && memoria->marcos[i].numero_pagina == numero_pagina) {
            ...
            log_info(logger, "## Query %d: Leído de memoria - Marco %d, Offset %d: %s", 
                     query_id, i, offset, datos_leidos);
            
            char comando_read[512];
            sprintf(comando_read, "READ_DATA|%d|%s", query_id, datos_leidos);
...
void flush_file_to_storage(t_worker_config *cfg, t_log *logger, int query_id, t_memoria_interna *memoria, const char *file, const char *tag)
{
    for (int i = 0; i < memoria->cant_marcos; i++) {
        if (memoria->marcos[i].ocupada && 
            ...
            sprintf(comando_flush, "FLUSH|%s|%s|%d|%.*s", 
                    file, tag, memoria->marcos[i].numero_pagina, 
