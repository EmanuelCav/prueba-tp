...
comando_t parse_comando(const char *cmd) {
    if (strncmp(cmd, "GET_BLOCK_SIZE", 14) == 0) return CMD_GET_BLOCK_SIZE;
    if (strncmp(cmd, "GET_FS_SIZE", 11) == 0) return CMD_GET_FS_SIZE;
    if (strncmp(cmd, "CREATE", 6) == 0) return CMD_CREATE;
    if (strncmp(cmd, "TRUNCATE", 8) == 0) return CMD_TRUNCATE;
    if (strncmp(cmd, "WRITE", 5) == 0) return CMD_WRITE;
    if (strncmp(cmd, "READ", 4) == 0) return CMD_READ;
    if (strncmp(cmd, "TAG", 3) == 0) return CMD_TAG;
    if (strncmp(cmd, "COMMIT", 6) == 0) return CMD_COMMIT;
    if (strncmp(cmd, "DELETE", 6) == 0) return CMD_DELETE;
    if (strncmp(cmd, "END", 3) == 0) return CMD_END;
    return CMD_UNKNOWN;
}
